...
game = hlt.Game("Discovery 0.8 EARLY S")
...
while True:
    ...
    if EARLY_GAME:
        ...
        for ship in my_ships:
            ...
            for e_ship in enemy_ships:
                ship_to_enemy_distance = ship.calculate_distance_between(
                    e_ship)
        ...
        # calculate best planets to go to
        # if turn == 0:
        ...
        if ships_in_critical_zone_count == 1:
            ...
            my_ships_sorted = sorted(
                my_ships, key=lambda ship: ship.calculate_distance_between(closest_enemy_ship))
        ...
        if not RUSH_MODE:
            ...
            for ship in my_ships:
                ...
                if allow_docking_for_ship[ship.id]:
                    ...
                    planet_priority_list = get_planets_by_distance(
                        game_map, ship)
                    ...
                    for planet in planet_priority_list:
                            ...
                            if planet.all_docked_ships()[0].owner == game_map.get_me() and not planet.is_full():
                                if ship.can_dock(planet):
                                    ...
                                    [navigate_command, (x, y)] = ship.navigate(ship.closest_point_to(planet), game_map, speed=int(
                                        hlt.constants.MAX_SPEED), max_corrections=120, new_ship_positions=new_ship_positions)
                        ...
                        # planet has no owner
                            ...
                            if planets_owned > 0 and len(my_ships) <= 3:
                                continue
                            if ship.can_dock(planet):
                                logging.info("DOCKING FREE PLANET")
                                command_queue.append(ship.dock(planet))
                                new_ship_positions.append((ship.x, ship.y))
                                break
                            else:
                ...
                # this ship is the only one to protect own ships
                    ...
                    if closest_enemy_dist < hlt.constants.EARLY_GAME_PROTECTION_RADIUS:
                        ...
                        [navigate_command, (x, y)] = ship.navigate(ship.closest_point_to(closest_enemy_ship), game_map, speed=int(
                            hlt.constants.MAX_SPEED), max_corrections=120, new_ship_positions=new_ship_positions)
                        new_ship_positions.append((x, y))
                        if navigate_command:
                            command_queue.append(navigate_command)
                    ...
                    # if no enemy ships are in radius of 40, move to closest ally ship with safety distance of 15
                        ...
                        my_ships_sorted = sorted(
                            my_ships, key=lambda ally: ally.calculate_distance_between(ship))

                        if len(my_ships_sorted) == 0:
                            continue

                        ally_ship = my_ships_sorted[0]
                        own_distance = ship.calculate_distance_between(
                            ally_ship)
                        ...
                        if own_distance == hlt.constants.EARLY_GAME_ALLY_RADIUS:
                            ...
                            x = own_x + \
                                math.cos(
                                    angle) * (hlt.constants.EARLY_GAME_ALLY_RADIUS - own_distance)
                            y = own_y + \
                                math.sin(
                                    angle) * (hlt.constants.EARLY_GAME_ALLY_RADIUS - own_distance)
        ...
        for ship in game_map.get_me().all_ships():
            ...
            for enemy in enemy_by_distance:
                ...
                [navigate_command, (x, y)] = ship.navigate(ship.closest_point_to(
                    enemy), game_map, speed=ship_speed, max_corrections=120, new_ship_positions=new_ship_positions)
                new_ship_positions.append((x, y))
        ...
        for ship in game_map.get_me().all_ships():
            ...
            for entity in entities_by_distance:
                ...
                if isinstance(entity, hlt.entity.Planet):
                    ...
                    if len(enemy_planets) > 0 and (len(planets) > 0 and planets_owned <= len(planets)):
                        '''if ships_following_enemy[enemy_ship.id] >= 2:
